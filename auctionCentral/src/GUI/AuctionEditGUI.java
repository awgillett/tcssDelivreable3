...
public class AuctionEditGUI extends JFrame{
	...
	DefaultTableModel tblModel = new DefaultTableModel(
			...
			new String[] {
				"ID", "Name", "Description", "Condition", "Min Bid"
			}
		);
...
//	}
	...
	private void initialize() {
		...
		JButton btnEditItem = new JButton("Edit Selected Item");
		btnEditItem.setToolTipText("Click here to edit items, add items, remove items, or cancel the auction.");
		btnEditItem.setBounds(338, 411, 154, 23);
	...
	private void populateAuctionInfo() {
		auctionInfo = "<html>Scheduled:  " + myAuction.getAuctionDate().format(dateFormat) + "<br>"
			    ...
			    + "To edit an item just double click on one in the list.</html>";
	}
	
	private void populateItems() {
		for (Item i : myAuction.getItemList()) {
			tblModel.addRow(new Object[]{i.getMyItemID(), i.getItemName(), i.getMyDescription(), i.getMyCondition(), currency.format(i.getMyMinBid())});
		}
	}
