...
public class BidderGUIaddBid extends JDialog {
		...
		btnAddABid.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (!txtBidAmount.getText().isEmpty()) {
					checkIfCanBid(txtBidAmount.getText());
				}
			}
		});
	...
	private boolean checkIfCanBid(String bidOffer) {
		...
		try {
			bid = Double.parseDouble(bidOffer);
		...
		int reply = currentBidder.addBidGUI(myCalendar, theItem, bid);
		int bidderResponse;

		switch (reply) {
		case bidAccepted:
			message = "You would like to place a bid of " + currency.format(bid) + " for "
					+ theItem.getItemName() + ".\n Please click yes to confirm or no to cancel.";
			bidderResponse = JOptionPane.showConfirmDialog(this, message);
			if (bidderResponse == 0) {
				currentBidder.placeBid(currentBidder.createBid(bid, theItem, myCalendar));
				close();
			}
			break;
		case bidBelowMinBid:
